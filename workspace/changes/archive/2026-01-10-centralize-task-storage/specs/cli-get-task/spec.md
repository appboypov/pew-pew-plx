...
- **WHEN** user runs `splx get task --id 001-add-feature-x-implement`
...
- **WHEN** user runs `splx get task --id 001-add-feature-x-implement --json`
...
- **WHEN** user runs `splx get tasks --parent-id add-feature --parent-type change`
- **THEN** displays tasks matching both parent-id and parent-type

#### Scenario: Parent ID conflict

- **WHEN** user runs `splx get tasks --parent-id my-entity`
...
- **WHEN** user runs `splx get task --parent-id add-feature`
...
- **WHEN** user runs `splx get task --parent-id add-feature --parent-type change`
