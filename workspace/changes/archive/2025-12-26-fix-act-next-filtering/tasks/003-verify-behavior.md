...
Manual verification confirms that `splx act next` correctly filters completed changes and outputs full task file content.
...
Running `splx act next` in the OpenSplx repository:
...
- [ ] `splx act next` skips 100% complete changes in OpenSplx
- [ ] `splx act next` returns an incomplete change
- [ ] Full task file content is displayed (not just checkboxes)
- [ ] Output order is: proposal.md → design.md (optional) → task content
- [ ] `splx act next --json` includes full `taskContent` field

## Implementation Checklist

- [x] 3.1 Run `splx act next` and verify it skips completed changes
...
- [x] 3.4 Run `splx act next --json` and verify `taskContent` contains full content
- [x] 3.5 Verify proposal.md is output before task content
- [x] 3.6 Run `npm run lint` to verify code style

## Notes

Current repository state (from `splx list`):
...
After this change, `splx act next` should select `add-scaffold-command` or similar incomplete change instead of a completed one.
