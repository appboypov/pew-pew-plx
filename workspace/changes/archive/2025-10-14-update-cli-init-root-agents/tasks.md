## 1. Implementation
- [x] 1.1 Refactor `splx init` to always generate the root `AGENTS.md` stub (initial run and extend mode) via shared helper logic.
...
- [x] 2.2 Execute `splx validate update-cli-init-root-agents --strict`.
- [x] 2.3 Perform a manual smoke test: run `splx init` in a temp directory, confirm stub + grouped prompts, rerun in extend mode.
