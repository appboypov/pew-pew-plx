...
- **Complete command documentation** with all current functionality:
  - `plx init [path]` - Initialize PLX in a project
  - `plx list` - List all active changes (default)
  - `plx list --specs` - List all specifications
  - `plx show [item]` - Display change or spec with auto-detection
  - `plx show` - Interactive mode for selection
  - `plx diff [change]` - Show spec differences for a change
  - `plx validate [item]` - Validate changes or specs
  - `plx archive [change]` - Archive completed change after deployment
  - `plx update [path]` - Update PLX instruction files
...
- **Remove deprecated command references** (noun-first patterns like `plx change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `plx show [change] --json --deltas-only` for inspecting deltas
  - `plx validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `plx show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `plx list --specs` to see all current specs
...
- Affected documentation:
  ...
  - CLAUDE.md files generated by `plx init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `plx archive update-agent-instructions --skip-specs`.