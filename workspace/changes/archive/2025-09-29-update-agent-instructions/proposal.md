# Update PLX Agent Instructions

## Why

The current PLX agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `splx init [path]` - Initialize PLX in a project
  - `splx list` - List all active changes (default)
  - `splx list --specs` - List all specifications
  - `splx show [item]` - Display change or spec with auto-detection
  - `splx show` - Interactive mode for selection
  - `splx diff [change]` - Show spec differences for a change
  - `splx validate [item]` - Validate changes or specs
  - `splx archive [change]` - Archive completed change after deployment
  - `splx update [path]` - Update PLX instruction files
...
- **Remove deprecated command references** (noun-first patterns like `splx change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `splx show [change] --json --deltas-only` for inspecting deltas
  - `splx validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `splx show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `splx list --specs` to see all current specs
...
- Affected documentation:
  - `workspace/README.md` - Main PLX instructions
  - CLAUDE.md files generated by `splx init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `splx archive update-agent-instructions --skip-specs`.
