...
The `splx paste` command currently only supports `splx paste request` for capturing clipboard content. Extending it to support all entity types (task, change, spec) provides parity with the planned `splx create` command but for clipboard-based workflows. Users with content in their clipboard (e.g., from issue trackers, documentation, or notes) can quickly scaffold entities without manual transcription.

## What Changes

**New Subcommands**
- `splx paste task` - Create task from clipboard content
- `splx paste task --parent-id <id>` - Create parented task from clipboard content
- `splx paste change` - Create change from clipboard content
- `splx paste spec` - Create spec from clipboard content
- `splx paste request` - (existing, unchanged)

**Shared Template System**
- Reuse entity templates from `splx create` command for consistent scaffolding
