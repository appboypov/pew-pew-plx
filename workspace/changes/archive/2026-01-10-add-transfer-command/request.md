...
8. **Review transfer**: Yes, include `splx transfer review` with same cascade behavior (review + linked tasks).
9. **Request transfer**: Yes, include `splx transfer request` for transferring request.md files.
...
11. **Workspace auto-init**: Follow existing `splx init` behavior - prompt interactively for confirmation, use source workspace's tool configuration as defaults. Respects --no-interactive and --yes flags.
...
Add a `splx transfer` command with subcommands for transferring PLX entities between workspaces.

### Commands

```
splx transfer change --id <id> [--source <path>] [--target <path>] [--target-name <name>] [--dry-run] [--yes]
splx transfer spec --id <id> [--source <path>] [--target <path>] [--target-name <name>] [--dry-run] [--yes]
splx transfer task --id <id> [--source <path>] [--target <path>] [--target-name <name>] [--dry-run] [--yes]
splx transfer review --id <id> [--source <path>] [--target <path>] [--target-name <name>] [--dry-run] [--yes]
splx transfer request --id <id> [--source <path>] [--target <path>] [--target-name <name>] [--dry-run] [--yes]
```
...
4. **Workspace auto-init**: If target has no workspace, initialize using `splx init` behavior with source's tool config as defaults
