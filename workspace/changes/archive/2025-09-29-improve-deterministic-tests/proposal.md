...
state via `process.cwd()` and `workspace/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `workspace/changes/sample-change/proposal.md`
  - `workspace/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `workspace/changes` during execution.
