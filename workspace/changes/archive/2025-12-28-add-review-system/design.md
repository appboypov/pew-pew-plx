...
5. Create workspace/reviews/<review-name>/
...
Add `--reviews` mode that scans `workspace/reviews/` like changes.

### Archive Command (src/core/archive.ts)

Extend to detect entity type:
1. Check if ID exists in `workspace/changes/` → archive as change
2. Check if ID exists in `workspace/reviews/` → archive as review
