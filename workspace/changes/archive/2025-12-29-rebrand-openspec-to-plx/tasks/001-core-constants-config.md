...
- `src/utils/command-name.ts`: defaults to "splx"
- `src/core/global-config.ts`: uses `~/.splx/` directory
...
- [x] Global config directory must be ~/.splx/
...
- [x] 1.3 Update `src/utils/command-name.ts`: change default from "openspec" to "splx"
- [x] 1.4 Update `src/core/global-config.ts`: change `GLOBAL_CONFIG_DIR_NAME` from "openspec" to "splx"
