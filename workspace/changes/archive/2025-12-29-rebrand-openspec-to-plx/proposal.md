...
OpenSplx is a fork that needs a distinct identity to coexist with the original OpenSpec. Currently, the codebase mixes "OpenSpec" and "PLX" terminology, with both CLI commands available. Users expect `splx` to be the sole command, with all directories and documentation using PLX terminology.

## What Changes

- **BREAKING**: Remove `openspec` CLI command entirely - only `splx` command remains
...
- **BREAKING**: Rename global config directory from `~/.openspec/` to `~/.splx/`
...
- Rename `openspec-conventions` spec to `splx-conventions`

## Impact

- Affected specs: `splx-conventions` (renamed from `openspec-conventions`)
- Affected code: All source files, templates, tests, and configuration files
- Breaking changes: Users must use `splx` command instead of `openspec`
