...
- **WHEN** user runs `splx get task --id project-a/001-implement`
...
- **WHEN** user runs `splx get task --json` in multi-workspace mode
...
- **WHEN** user runs `splx get task --workspace project-a`
...
- **WHEN** user runs `splx get tasks --id project-a/add-feature`
