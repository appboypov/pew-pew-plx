...
The command SHALL configure AI coding assistants with PLX instructions based on user selection.

#### Scenario: Prompting for AI tool selection

- **WHEN** run
- **THEN** prompt user to select AI tools to configure:
  - Claude Code (✅ PLX custom slash commands available)
  - Cursor (✅ PLX custom slash commands available)
...
- **THEN** create or update `AGENTS.md` in the project root directory (not inside workspace/)
...
See @workspace/AGENTS.md for detailed conventions and guidelines.
<!-- PLX:END -->
```
...
- **THEN** create new file with PLX content wrapped in markers using the same template as CLAUDE.md
...
- **THEN** preserve all existing content
- **AND** ensure the PLX-managed block at the beginning of the file is refreshed without duplicating markers

#### Scenario: Managing content with markers

- **WHEN** using the marker system
- **THEN** use `<!-- PLX:START -->` to mark the beginning of managed content
- **AND** use `<!-- PLX:END -->` to mark the end of managed content
- **AND** allow PLX to update its content without affecting user customizations
