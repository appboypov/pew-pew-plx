...
Workspace maintenance is manual and easy to forget. Changes accumulate in various states (open, in-progress, done-but-not-archived) without a systematic way to assess their health and take action. A `/splx/sync-workspace` command provides a single entry point for workspace hygiene.

## What Changes

- Add new slash command `/splx/sync-workspace` for workspace maintenance
...
- Affected code:
  ...
  - `.claude/commands/splx/sync-workspace.md` (new command file)
