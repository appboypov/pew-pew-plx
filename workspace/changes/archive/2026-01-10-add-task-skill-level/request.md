...
- The splx/orchestrate command should be updated to instruct to choose the appropriate model based on task level if present, otherwise determine itself
...
3. **Orchestrate command update**: Update splx/orchestrate.md to include model selection logic based on skill level
...
5. Should splx/plan-proposal auto-assign skill levels to generated tasks?
...
5. **Auto-assignment**: `splx/plan-proposal` should auto-assign skill levels based on task complexity heuristics.
6. **Sub-agent only**: Model selection based on skill level only applies when spawning sub-agents (e.g., in splx/orchestrate). The executing model cannot change itself.
...
2. **CLI output**: Display skill level in `splx get task` and `splx get tasks` (human + JSON)
...
5. **splx/plan-proposal**: Auto-assign skill levels based on complexity heuristics
6. **splx/orchestrate**: Select sub-agent model based on skill level (haiku/sonnet/opus for Claude; equivalent for other models)
