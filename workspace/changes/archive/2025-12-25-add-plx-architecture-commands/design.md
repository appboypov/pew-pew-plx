...
**Goals:**
- PLX slash commands installed during `splx init` alongside PLX commands
...
**Decision:** Archive command suggests running `/splx/update-architecture` but does not execute it automatically.
...
**Rationale:** Consistency with existing marker system, allows `splx update` to refresh PLX content if needed in the future.
...
2. Running `splx init` in extend mode adds PLX commands to existing projects
