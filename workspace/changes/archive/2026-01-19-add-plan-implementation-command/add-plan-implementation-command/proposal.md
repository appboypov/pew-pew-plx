...
Current multi-agent workflows lack a structured handoff protocol. The `/splx:orchestrate` command keeps agents in-process, but users need to manually coordinate between separate agent sessions. This change introduces a `plan-implementation` workflow that generates copy-pastable task blocks for external agents with full context.

## What Changes

- **NEW** CLI command `splx create progress --change-id <id>` - Generates PROGRESS.md with non-completed tasks
- **NEW** Slash command `/splx:plan-implementation` - Orchestrates multi-agent handoff workflow
...
- Affected specs: `cli-create`, `splx-slash-commands`
