...
- [x] 2.4 When no `[spec-id]` provided and interactivity allowed, prompt to select from `workspace/specs/*/spec.md` and validate the selected spec
...
- [x] 3.2 Create `src/commands/validate.ts` implementing:
  ...
  - [x] 3.2.6 Bulk validation for `--all`, `--changes`, `--specs` (exclude `workspace/changes/archive/`)
...
- [x] 8.3 Ensure existing specs under `workspace/changes/bulk-validation-interactive-selection/specs/*` remain satisfied
