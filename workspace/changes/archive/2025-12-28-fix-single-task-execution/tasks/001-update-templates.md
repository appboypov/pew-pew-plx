...
- `splx-slash-command-templates.ts` Step 4 says "get next task" implying automatic continuation
...
- [ ] `getTaskSteps` in `splx-slash-command-templates.ts` includes explicit stop instruction
- [ ] Stage 2 in `agents-template.ts` replaces "Repeat" with stop-and-wait instruction
- [ ] TypeScript compiles without errors

## Implementation Checklist

- [x] 1.1 Update `getTaskSteps` in `src/core/templates/splx-slash-command-templates.ts` to replace step 4 with explicit completion and stop instructions
...
- `getTaskSteps` in `splx-slash-command-templates.ts` (lines 29-35)
