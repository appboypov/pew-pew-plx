...
- **WHEN** executing `splx show` without arguments
...
- **WHEN** executing `splx show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `splx show <item>` or `splx change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `splx show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `splx change show`/`splx spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `splx show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `splx show --type spec <item>`
...
- **WHEN** showing a change with `splx show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `splx show <spec-id> --requirements`
