...
2. Add `--specs`/`--changes` to `splx list`, with `--changes` as default.
3. Show deprecation warnings for `splx change list` and, more generally, for any `splx change ...` and `splx spec ...` subcommands.
...
## Out of Scope
JSON output parity for `splx list` across modes and `show --specs/--changes` discovery are follow-ups.
