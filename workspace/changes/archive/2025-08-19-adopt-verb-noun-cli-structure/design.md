...
2. Add `--specs`/`--changes` to `plx list`, with `--changes` as default.
3. Show deprecation warnings for `plx change list` and, more generally, for any `plx change ...` and `plx spec ...` subcommands.
...
## Out of Scope
JSON output parity for `plx list` across modes and `show --specs/--changes` discovery are follow-ups.
