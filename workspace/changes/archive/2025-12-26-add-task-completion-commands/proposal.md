...
- Add `splx complete task --id <task-id>` command to mark a task as done
- Add `splx complete change --id <change-id>` command to complete all tasks in a change
- Add `splx undo task --id <task-id>` command to revert a task to to-do
- Add `splx undo change --id <change-id>` command to revert all tasks in a change
- Modify `splx get task` to auto-transition to-do tasks to in-progress when retrieved
...
- Affected docs: `workspace/AGENTS.md`, `.claude/commands/splx/`
