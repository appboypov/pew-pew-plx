...
The `splx paste spec` command creates a specification directory from clipboard content.

## Currently

The `PasteCommand` class only supports `splx paste request`. There is no way to scaffold a spec from clipboard content.

## Should

- `splx paste spec` creates spec directory in `workspace/specs/`
...
- [ ] Spec structure matches `splx create spec` output (assumes `add-create-command` complete)
...
- [ ] `splx paste spec` creates spec directory with spec.md
