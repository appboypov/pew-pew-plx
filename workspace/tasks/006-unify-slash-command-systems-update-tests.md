...
## Currently
- `splx-parity.test.ts` tests PLX/regular parity (no longer needed)
...
## Should
- `splx-parity.test.ts` is deleted
...
- [x] 6.1 Delete `test/core/configurators/slash/splx-parity.test.ts`
...
## Notes
Search for `SplxSlashCommand` and `splx-` to find all references.
