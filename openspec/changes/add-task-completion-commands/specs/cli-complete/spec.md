...
- **WHEN** user runs `openspec complete task --id <task-id>`
...
- **WHEN** user runs `openspec complete task --id <task-id>` on a task with status 'done'
- **THEN** a warning message is displayed
- **THEN** the command exits with code 0

#### Scenario: Complete non-existent task

- **WHEN** user runs `openspec complete task --id <invalid-id>`
...
- **WHEN** user runs `openspec complete change --id <change-id>`
...
- **WHEN** user runs `openspec complete change --id <invalid-id>`
- **THEN** an error message is displayed
- **THEN** the command exits with code 1
...
- **WHEN** user runs `openspec complete task --id <task-id> --json`
...
- **WHEN** user runs `openspec complete change --id <change-id> --json`
