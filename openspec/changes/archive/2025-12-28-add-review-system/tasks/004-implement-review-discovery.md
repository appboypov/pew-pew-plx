...
- [ ] getActiveReviewIds() returns review IDs from openspec/reviews/
- [ ] getActiveReviewIds() excludes archive/ subdirectory
- [ ] getActiveReviewIds() only includes directories with review.md
- [ ] getArchivedReviewIds() returns review IDs from openspec/reviews/archive/
...
export async function getActiveChangeIds(root: string = process.cwd()): Promise<string[]> {
  const changesDir = path.join(root, OPENSPEC_DIR_NAME, 'changes');
  // ... scan and filter
}
```
